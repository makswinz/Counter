...
public static class StartupService
{
    ...
    /// <summary>What the entry was called when the application had a different name.</summary>
    private const string LegacyValueName = "FocusNotch";
    ...
    public static void RefreshPath()
    {
        try
        {
            ...
            // The entry used to be called something else. Carry the setting across rather than
            // leaving a dead value pointing at an executable that no longer exists, and leaving
            // the user wondering why the application stopped starting with Windows.
            if (key.GetValue(ValueName) is not string && key.GetValue(LegacyValueName) is string legacy
                && legacy.Length > 0)
            {
                key.SetValue(ValueName, legacy, RegistryValueKind.String);
                key.DeleteValue(LegacyValueName, throwOnMissingValue: false);
                Log.Info("Renamed the Windows startup entry to " + ValueName + ".");
            }
