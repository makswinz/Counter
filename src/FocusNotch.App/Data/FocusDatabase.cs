...
/// <summary>
/// Owns the single SQLite connection for the process. Focus Notch is a single-user desktop
...
public sealed class FocusDatabase : IDisposable
{
    ...
    public FocusDatabase(string databasePath)
    {
        ...
        catch (Exception ex)
        {
            throw new DatabaseUnavailableException(
                "Could not open the Focus Notch database at " + databasePath + ".", ex);
        }
    }
    ...
    public void Migrate()
    {
        lock (_gate)
        {
            ...
            {
                throw new DatabaseUnavailableException(
                    "This database was written by a newer version of Focus Notch (schema " + version +
