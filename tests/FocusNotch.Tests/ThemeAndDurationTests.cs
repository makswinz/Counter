...
using FocusNotch.Core.Validation;
using Xunit;
...
public class ThemeTests
{
    ...
    public void Every_state_ramp_is_actually_used_somewhere()
    {
        ...
        var sources = ThemeAndViewFiles()
            .Concat(new[]
            {
                Path.Combine(RepositoryRoot(), "src", "FocusNotch.App", "Views", "NotchWindow.xaml.cs")
            })
    ...
    private static IEnumerable<string> ThemeAndViewFiles()
    {
        ...
        foreach (var file in Directory.GetFiles(Path.Combine(root, "src", "FocusNotch.App", "Theme"), "*.xaml"))
        {
            yield return file;
        }

        foreach (var file in Directory.GetFiles(Path.Combine(root, "src", "FocusNotch.App", "Views"), "*.xaml"))
        {
            yield return file;
        }
    }
    ...
    private static string BrushesFile() =>
        Path.Combine(RepositoryRoot(), "src", "FocusNotch.App", "Theme", "Brushes.xaml");
