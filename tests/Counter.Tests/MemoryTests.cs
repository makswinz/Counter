...
public class MemoryTests
{
    ...
    public void A_backup_is_taken_at_most_once_a_day()
    {
        ...
        Assert.Equal(2, Directory.GetFiles(backups.Path, "counter-*.db").Length);
    }

    [Fact]
    public void Only_the_seven_most_recent_backups_are_kept()
    {
        // This one caught a real bug rather than a hypothetical: SQLite connections are pooled by
        // default, so a backup file stayed open after being written and every attempt to delete
        // it failed silently. The folder grew without limit. It depended on timing, so it passed
        // on one machine for weeks and failed the first time it ran on a clean one.
        var settings = new FakeSettingsStore();
        using var backups = new TempDirectory();
        using var db = new TempDatabase();
        ...
        Assert.Equal(
            ...
            Directory.GetFiles(backups.Path, "counter-*.db").Length);
    }
    ...
    public void A_backup_does_not_disturb_the_live_database()
    {
        ...
        var copy = Directory.GetFiles(backups.Path, "counter-*.db").Single();
    ...
    private sealed class TempDirectory : IDisposable
    {
        public TempDirectory()
        {
            Path = System.IO.Path.Combine(
                System.IO.Path.GetTempPath(), "counter-tests", Guid.NewGuid().ToString("N"));
